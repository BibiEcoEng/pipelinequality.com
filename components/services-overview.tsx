  ...
  return (
    <section className="bg-white py-8 md:py-20 lg:py-28" id="services">
      <div className="max-w-[1352px] mx-auto px-4 md:px-12 lg:px-20">
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-12 lg:gap-16 mb-16">
          {/* Column 1 - Field Inspection */}
          <div className="space-y-6">
            <div className="rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-shadow mb-6">
              <img
                src="/ndt_close.png"
                alt="Field Inspection & Coordination"
              ...
              <ul className="space-y-3">
                {(t.raw('sections.fieldInspection.items') as string[]).map((item, idx) => (
                  <li key={idx} className="flex items-start gap-3">
                    <Check className="w-5 h-5 text-muted-gold flex-shrink-0 mt-0.5" strokeWidth={2.5} />
        ...
        <div className="bg-gray-50 rounded-lg p-8 md:p-12">
          <h3 className="text-xl md:text-2xl font-bold text-navy mb-6">{t('keyAreasTitle')}</h3>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {serviceAreas.map((area, index) => (
              <div key={index} className="flex items-start gap-3">
                <Check className="w-5 h-5 text-muted-gold flex-shrink-0 mt-0.5" strokeWidth={2.5} />
                <span className="text-charcoal text-sm md:text-base font-medium">{area}</span>
              </div>
            ))}
          </div>
        </div>
